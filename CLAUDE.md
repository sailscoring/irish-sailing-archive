...
7. **Never edit `identities.json` by hand, and never change a slug.** It is
   ...
   `identity-curation.json`, which survives regeneration. The previous
   `identities.json` contributes **slugs and nothing else** — name, club and
   note are re-derived every run, so a curated rename actually lands. A
   display name is corrected here even for an identity the app has merged a
   live row into: the archive identity is the one that survives a merge, and
   it carries this repo's label. A slug is a public
